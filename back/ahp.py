...
def EVM_ranking(M: np.ndarray) -> np.ndarray:
    if M.size == 1:
        return np.array([1])
...
def make_criteria_ranking(criteria_matrix: np.ndarray) -> np.ndarray:
    return EVM_ranking(criteria_matrix)
...
def make_propositions_criteria_rankings(
    ...
    for i in range(propositions_matrices.shape[0]):
        propositions_rankings[i, :] = EVM_ranking(propositions_matrices[i, :, :])
